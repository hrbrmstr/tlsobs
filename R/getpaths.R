#' Retrieve paths from a certificate to one of multiple roots.
#'
...
#' @return a `list` containing the paths document. Each entry in the path contains the
#'     current certificate and an array of parents, if any exist.
#' @references <https://github.com/mozilla/tls-observatory#get-apiv1paths>
#' @export
#' @examples
#' str(get_paths(1))
get_paths <- function(id, sha256, host=get_obs_host(), timeout=60) {
  ...
  res <- httr::GET(sprintf("https://%s/api/v1/paths", host[1]),
                   query = q,
                   timeout(timeout))
